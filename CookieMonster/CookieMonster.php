...
class CookieMonster extends CWidget
{
     ...
     * source:          string which message source application component to use as in Yii::t() $source, default null
     */
    ...
    /**
     * Initialises the widget, registers the js file
     */
    public function init()
    {
        $assets = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'assets';
        Yii::app()->getClientScript()->registerScriptFile(Yii::app()->getAssetManager()->publish($assets) . '/CookieMonster.js', CClientScript::POS_END);
    ...
    protected function initCookie()
    {
        ...
        $cookieOptions = json_encode($cookieOptions);
        Yii::app()->getClientScript()->registerScript('CookieMonsterInit', "CookieMonster.init($cookieOptions);", CClientScript::POS_END);
    }
    ...
    protected function setMode()
    {
        ...
        switch ($this->mode) {
            case 'bottom':
                $this->outerStyle = array_merge($this->outerStyle, array(
                    'bottom'     => 0,
                    'left'       => 0,
                    'width'      => '100%',
